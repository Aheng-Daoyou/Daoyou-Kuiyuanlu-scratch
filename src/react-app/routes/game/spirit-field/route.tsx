...
const stageNames = { germination: '萌芽期', nourishing: '蕴灯期', forming: '成型期' } as const;
const statusText = { empty: '空田', awaiting_cultivation: '静候施为', growing: '生长中', ready_to_harvest: '待摘取' } as const;
const affinityNames: Record<string, string> = { excellent: '天性相合', good: '灯机相契', neutral: '平稳承纳', strained: '灯机滞涩' };
...
export default function SpiritFieldPage() {
  ...
  const mutateField = useCallback(async <T,>(url: string, body: unknown): Promise<T | null> => {
    ...
    try {
      ...
      await refresh();
      return result;
    } catch (next) { pushToast({ message: next instanceof Error ? next.message : '灯田操作失败', tone: 'danger' }); return null; }
  ...
  const cultivate = async (method: Method) => {
    ...
    if (!result) return;
    openDialog({ title: `${result.methodName} · ${affinityNames[result.affinity] ?? '灯机已动'}`, content: <div className="space-y-2 text-sm leading-7"><p>{result.feedback}</p><p className="text-ink-secondary">本阶段预计需要{duration(result.durationMs)}。阶段结束前无需重复操作。</p></div>, confirmLabel: '记下变化', onConfirm: async () => undefined });
  };
  const harvest = async () => {
    if (!selected) return;
    ...
    if (!result) return;
    openDialog({ title: `${result.name} · ${result.quality}`, content: <div className="space-y-2 text-sm leading-7"><p>{result.description}</p><p className="text-ink-secondary">收获 ×{result.quantity}{result.mutated ? '，造化升品' : result.degraded ? '，灯韵稍退' : ''}</p></div>, confirmLabel: '收入囊中', onConfirm: async () => undefined });
  };

  if (loading) return <GameSceneLoading message="正推开灯宅药圃的竹门……" />;
  ...
  return (
    <GameSceneFrame
      variant="workflow"
      headerMeta={<div className="flex flex-wrap gap-x-5 gap-y-1 text-sm text-ink-secondary"><span>境界：{snapshot.player.realm}</span><span>灯油：{snapshot.player.qi}/{snapshot.player.qiMax}</span><span>灯焰：{snapshot.player.mp}/{snapshot.player.mpMax}</span><span>成功收获：{snapshot.profile.successfulHarvestCount}</span></div>}
      aside={<div className="space-y-3 text-sm leading-6"><p className="text-ink-secondary">储物袋中的灯种只显露些许天性，真正形态要等三度造化走完才会揭晓。</p>{!snapshot.profile.starterClaimed ? <InkButton disabled={busy} onClick={() => void mutateField('/api/spirit-field/starter', {})} variant="primary">领取初始灯种</InkButton> : usableSeeds.length ? usableSeeds.map((seed) => <div key={seed.materialId} className="flex justify-between gap-3"><span>{seed.name} · {seed.quality}</span><span>×{seed.quantity}</span></div>) : <span className="text-ink-secondary">暂无灯种。</span>}</div>}
    >
      {error ? <InkNotice tone="warning">{error}</InkNotice> : null}
      <InkCard padding="lg" className="space-y-3">
        <p className="text-sm text-ink-secondary">六畦灯土皆已开垦。选中一畦，完成当前阶段唯一一次培育选择；错过操作时，灯植会停在原处等你归来。</p>
      ...
      {selected ? <InkCard padding="lg" className="space-y-4">
        ...
        : <div className="space-y-4">
          ...
          {selected.history.length ? <div className="space-y-2 border-ink/15 border-t border-dashed pt-3">{selected.history.map((item) => <div key={item.stage}><div className="text-xs text-crimson">{stageNames[item.stage as keyof typeof stageNames]} · {affinityNames[item.affinity] ?? '灯机已动'}</div><p className="text-sm leading-6 text-ink-secondary">{item.feedback}</p></div>)}</div> : null}
          {selected.status === 'awaiting_cultivation' ? <div className="space-y-3"><GameSceneNote>灯机已停在{selected.stage ? stageNames[selected.stage] : '当前阶段'}，选定一次培育方式后才会继续生长。</GameSceneNote><div className="grid gap-3 sm:grid-cols-2">{selected.methods.map((method) => {
            ...
            const costText = method.resourceKind === 'none' ? '无需额外资源' : method.resourceKind === 'qi' ? `灯油 ${method.cost.amount}` : method.resourceKind === 'mp' ? `灯焰 ${method.cost.amount}` : method.resourceKind === 'spirit_stones' ? `灯油券 ${method.cost.amount}` : `${method.cost.amount} 份所选物品${method.cost.spiritStones ? ` + 灯油券 ${method.cost.spiritStones}` : ''}`;
          ...
          {selected.status === 'ready_to_harvest' ? <div className="space-y-3"><GameSceneNote>三度造化已定，最终形态不可逆转。采摘后才会揭晓它成为灯草、奇珍异物还是灯果。</GameSceneNote><InkButton variant="primary" disabled={busy} onClick={() => void harvest()}>采摘造化产物</InkButton></div> : null}
        </div>}
      </InkCard> : null}
    </GameSceneFrame>
  );
