...
import { BAIXIBAN_ORGANIZATION } from './BaixibanOrganizationModule';
...
describe('BaixibanOrganizationModule', () => {
  it('centralizes V1 facility permissions by disciple rank', () => {
    const registered =
      BAIXIBAN_ORGANIZATION.capabilities.snapshot('registered');
    ...
    const outer = BAIXIBAN_ORGANIZATION.capabilities.snapshot('outer');
    ...
    const inner = BAIXIBAN_ORGANIZATION.capabilities.snapshot('inner');
  ...
  it('owns rank, economy, task, and construction content', () => {
    expect(
      ...
      ).map((rank) => BAIXIBAN_ORGANIZATION.ranks.methodLevelCap(rank)),
    ).toEqual([45, 90, 135, 180]);
    expect(BAIXIBAN_ORGANIZATION.ranks.requirement('outer')).toMatchObject({
      minRealm: '闻腥',
      contribution: 100,
      dailyCompletions: 3,
    });
    expect(BAIXIBAN_ORGANIZATION.ranks.requirement('inner')).toMatchObject({
      minRealm: '守灯',
      contribution: 500,
    });
    expect(BAIXIBAN_ORGANIZATION.ranks.requirement('true')).toMatchObject({
      minRealm: '蚀体',
      contribution: 3000,
    });
    expect(
      BAIXIBAN_ORGANIZATION.ranks.requirement('true').requiredTaskTags,
    ).toContainEqual({ tag: 'promotion.elder_trial', label: '通过长老试炼' });
    expect(BAIXIBAN_ORGANIZATION.tasks.get('gate_sweep')?.executorKey).toBe(
      'sect.sweep',
    );
    expect(BAIXIBAN_ORGANIZATION.construction.facilities[0]?.key).toBe('archive');
    expect(BAIXIBAN_ORGANIZATION.construction.upgradeTarget(1)).toBe(250);
    expect(
      [1, 2, 3, 4, 5].map((archiveLevel) =>
        BAIXIBAN_ORGANIZATION.benefits.methodLevelCap(
    ...
    expect(BAIXIBAN_ORGANIZATION.benefits.methodLevelCap(levels)).toBe(180);
    expect(
      BAIXIBAN_ORGANIZATION.benefits.retreatMultiplier(levels, 'outer'),
    ).toBe(1.1);
    expect(
      BAIXIBAN_ORGANIZATION.benefits.craftDiscount(
        'sect.craft.alchemy',
        levels,
        'true',
      ),
    ...
    expect(BAIXIBAN_ORGANIZATION.benefits.stipendMultiplier(levels)).toBe(1.25);
  });

  it('declares independent battle and material bounties', () => {
    const battle = BAIXIBAN_ORGANIZATION.tasks.get('weekly_bounty_battle');
    const material = BAIXIBAN_ORGANIZATION.tasks.get(
  ...
  it('builds the fixed mine beast at 75% of every realm-stage budget', () => {
    const factory = BAIXIBAN_ORGANIZATION.battles.get('mine_patrol')!;
    ...
    expect(
      Object.fromEntries(
        [
          ...
          taskId,
          BAIXIBAN_ORGANIZATION.battles.get(taskId)?.stateStrategy,
        ]),
      ),
    ).toEqual({
  ...
  it.each([
    ...
    (taskId, acquisition, title) => {
      ...
      expect(BAIXIBAN_ORGANIZATION.battles.get(taskId)?.acquisition).toBe(
        acquisition,
      );
  ...
  it('builds the fixed Baixiban elder combat loadout', () => {
    ...
    expect(result).toMatchObject({
      title: '长老试炼',
      presetId: 'elder-trial-baixiban-v1',
      opponent: {
        name: '听戏老人·试炼化身',
        realm: '蚀体',
        realm_stage: '圆满',
    ...
    expect(result?.opponent.inventory.artifacts).toMatchObject([
      { name: '照尘古器', slot: 'weapon', quality: '地品' },
      { name: '藏锋器衣', slot: 'armor', quality: '地品' },
      { name: '澄心器珏', slot: 'accessory', quality: '地品' },
    ]);
    expect(
