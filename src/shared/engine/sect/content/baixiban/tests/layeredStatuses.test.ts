...
describe('百戏班分层状态', () => {
  ...
  it.each([1, 2, 3])(
    '戏痕%i层提高对应伤害2.24%%/层并排除反伤与DOT',
    (layers) => {
      const target = new Unit('target', '目标', {});
      const caster = new Unit('caster', '施法者', {});
      const config = statusConfig('swift-sword', BAIXIBAN_STAGE_MARK_BUFF);
  ...
  it.each([1, 2, 3])('裂甲%i层降低3.36%%物防/层，移除后完整恢复', (layers) => {
    const target = new Unit('target', '目标', {});
    const caster = new Unit('caster', '施法者', {});
    const config = statusConfig('heavy-sword', BAIXIBAN_ARMOR_REND_BUFF);
    ...
    target.buffs.removeBuff(BAIXIBAN_ARMOR_REND_BUFF);
