...
import { BAIXIBAN_STAGE_GRACE } from '..';
...
function executeSkill(skill: ActiveSkill, caster: Unit, target: Unit, shouldApplyEffects?: boolean): void {
  skill.execute({
    ...
    resolution: createHitResolution({
      actionId: `${caster.id}:baixiban-test`,
      castId: `${skill.id}:baixiban-test`,
      caster,
      target,
    }),
  });
...
describe('百戏班参悟运行时语义', () => {
  ...
  it('极限照影收束将戏念与戏痕合并结算且不超过4段', () => {
    const { owner, enemy } = install('swift-sword', [
      'swift-mountain-breaking',
      'swift-endless-flow',
    ]);
    const linked = owner.abilities.getAbility('sect.baixiban.linked-edge') as ActiveSkill;
    for (let index = 0; index < 3; index += 1) executeSkill(linked, owner, enemy);
    owner.combatResources.set(BAIXIBAN_STAGE_GRACE, 6);
    const ultimate = owner.abilities.getAbility('sect.baixiban.sect-ultimate') as ActiveSkill;
  ...
  it('留痕使走场施加两层戏痕', () => {
    ...
    expect(
      enemy.buffs
        .getAllBuffs()
        .find((buff) => buff.id === 'sect.baixiban.sword-mark')
        ?.getLayer(),
    ).toBe(2);
  });
  ...
  it('回风的附加护盾每次压轴持续期间最多触发一次', () => {
    ...
    const dodge = (): void =>
      EventBus.instance.publish<DodgeEvent>({
        ...
        target: owner,
        ability: AbilityFactory.create(turning),
        resolution: createHitResolution({ actionId: 'baixiban:dodge:1', castId: 'baixiban:dodge:1', caster: enemy, target: owner }),
      });
    ...
    expect(owner.combatResources.getCurrent(BAIXIBAN_STAGE_GRACE)).toBe(1);
  });

  it('静潮在连续两个自身行动未收束后暂停衰减并强化下一次收束', () => {
    const { sect, owner, enemy } = install('swift-sword', ['swift-still-tide']);
    owner.combatResources.set(BAIXIBAN_STAGE_GRACE, 3);
    ...
    actionPost();
    owner.combatResources.finishAction(false, false);
    expect(owner.combatResources.getCurrent(BAIXIBAN_STAGE_GRACE)).toBe(3);
  ...
  it('承锋只降低整场战斗第一次直接伤害并获得2点戏念', () => {
    ...
    expect(owner.combatResources.getCurrent(BAIXIBAN_STAGE_GRACE)).toBe(2);
    damageSystem.destroy();
  });

  it('压轴把减伤与后发登记给施法者而不立即攻击目标', () => {
  ...
  it('快器藏锋未命中时整条普通效果链失效', () => {
    ...
    expect(owner.combatResources.getCurrent(BAIXIBAN_STAGE_GRACE)).toBe(0);
  });

  it('基础与重器藏锋不依赖前置命中并正常登记后发', () => {
    ...
    for (const abilityId of ['turning-body', 'shadow-step']) {
      ...
      const dodge = (): void =>
        EventBus.instance.publish<DodgeEvent>({
          ...
          target: owner,
          ability,
          resolution: createHitResolution({ actionId: 'baixiban:dodge:2', castId: 'baixiban:dodge:2', caster: enemy, target: owner }),
        });
