...
import { BAIXIBAN_MODULE } from '../BaixibanSectModule';
...
function runtimeFingerprint(
  ...
  const projection = productionSectRuntime.compiler.compile(BAIXIBAN_MODULE, {
  ...
  const defaultId = BAIXIBAN_MODULE.definition.abilities.find(
  ...
  const execute = (abilityId: string, target: Unit): ActiveSkill => {
    const ability = owner.abilities.getAbility(
      `sect.baixiban.${abilityId}`,
    ) as ActiveSkill;
    ability.execute({ caster: owner, target, resolution: createHitResolution({ actionId: `${owner.id}:baixiban:${abilityId}`, castId: `${ability.id}:baixiban`, caster: owner, target }) });
    return ability;
  };
  ...
  if (queued) {
    (AbilityFactory.create(queued.ability) as ActiveSkill).execute({
      caster: owner,
      target: enemy,
      resolution: createHitResolution({ actionId: `${owner.id}:baixiban:queued`, castId: `${queued.ability.slug}:baixiban`, caster: owner, target: enemy }),
    });
  }
  ...
  bus.publish<DodgeEvent>({
    type: 'DodgeEvent',
    resolution: createHitResolution({ actionId: 'baixiban:matrix:dodge', castId: 'baixiban:matrix:dodge', caster: enemy, target: owner }),
    timestamp: Date.now(),
    caster: enemy,
    target: owner,
    ability: turning,
  ...
  const incomingRequest: DamageSegmentRequestedEvent = {
    ...
    resolution: createHitResolution({ actionId: 'baixiban:matrix:incoming', castId: 'baixiban:matrix:incoming', caster: enemy, target: owner }),
    timestamp: Date.now(),
    caster: enemy,
    target: owner,
    ability: enemyAttack,
  ...
  bus.publish<DamageSegmentAppliedEvent>({
    ...
    resolution: createHitResolution({ actionId: 'baixiban:matrix:applied', castId: 'baixiban:matrix:applied', caster: enemy, target: owner }),
    timestamp: Date.now(),
    caster: enemy,
    target: owner,
    ability: enemyAttack,
  ...
  bus.publish<ShieldBreakEvent>({
    ...
    resolution: createHitResolution({ actionId: 'baixiban:matrix:break', castId: 'baixiban:matrix:break', caster: enemy, target: owner }),
    timestamp: Date.now(),
    caster: enemy,
    target: owner,
    ability: enemyAttack,
  ...
  const plain = owner.abilities.getAbility(
    'sect.baixiban.plain-sword',
  ) as ActiveSkill;
  for (let index = 0; index < 2; index += 1) {
    bus.publish<SkillCastEvent>({
      type: 'SkillCastEvent',
      resolution: createHitResolution({ actionId: `baixiban:matrix:plain:${index}`, castId: `baixiban:matrix:plain:${index}`, caster: owner, target: enemy }),
      timestamp: Date.now(),
      caster: owner,
      target: enemy,
      ability: plain,
  ...
  for (let index = 0; index < 2; index += 1) {
    bus.publish<ActionPostEvent>({
      ...
      resolution: createHitResolution({ actionId: `baixiban:matrix:post:${index}`, castId: `baixiban:matrix:post:${index}`, caster: owner, target: enemy }),
      timestamp: Date.now(),
      caster: owner,
    });
  }
  ...
  owner.combatResources.set(BAIXIBAN_STAGE_GRACE, 6);
  execute('sect-ultimate', enemy);
  execute('guiding-sword', enemy);
  plain.execute({ caster: owner, target: enemy, resolution: createHitResolution({ actionId: 'baixiban:matrix:plain-execute', castId: 'baixiban:matrix:plain-execute', caster: owner, target: enemy }) });
  bus.publish<RoundStartEvent>({
    type: 'RoundStartEvent',
    resolution: createHitResolution({ actionId: 'baixiban:matrix:round', castId: 'baixiban:matrix:round', caster: owner, target: owner }),
    timestamp: Date.now(),
    turn: 2,
  });
