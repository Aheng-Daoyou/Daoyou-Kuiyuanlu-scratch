...
describe('宗门注册投影', () => {
  it('九个神通跨境界保持固定蓝耗且亮相始终免费', () => {
    ...
    for (const realm of ['闻腥', '忘川', '渡渊'] as const) {
  ...
  it.each([
    ...
    [180, 4],
    [999, 4],
  ])('破台在%i级驱散%i个正面状态', (level, expectedCount) => {
  ...
  it('闻腥初期心法上限可解锁除绝式外全部基础神通，10级开放谢幕', () => {
    const early = state();
    early.methods = Object.fromEntries(
      Object.keys(early.methods).map((methodId) => [methodId, 5]),
    );
    expect(
      listUnlockedAbilityIds(BAIXIBAN_MODULE.definition, early),
    ).toHaveLength(9);
    expect(
      listUnlockedAbilityIds(BAIXIBAN_MODULE.definition, early),
    ).not.toContain('sect-ultimate');
    early.methods['baixiban-canon'] = 10;
    expect(listUnlockedAbilityIds(BAIXIBAN_MODULE.definition, early)).toContain(
      'sect-ultimate',
    );
  });

  it('炉火铸骨在基础态与两条流派中固定提供护持加成与受击蓄念', () => {
    for (const pathId of [undefined, 'swift-sword', 'heavy-sword'] as const) {
      const sect = state(pathId);
      ...
      expect(detail.config.tags).not.toContain(
        pathId ? GameplayTags.ABILITY.SECT.path('baixiban', pathId) : undefined,
      );
      const projection = projectSectCombat({ sect, realm: '忘川' })!;
      expect(
        projection.abilities.filter(
          (ability) => ability.slug === 'sect.baixiban.baixiban-runtime',
        ),
      ).toHaveLength(1);
    }
  });

  it('百戏班两条流派各自保持六层且每层三个节点', () => {
    for (const path of BAIXIBAN_MODULE.definition.paths) {
    ...
    const swift = BAIXIBAN_MODULE.definition.paths.find(
      (path) => path.id === 'swift-sword',
    )!;
    const heavy = BAIXIBAN_MODULE.definition.paths.find(
      (path) => path.id === 'heavy-sword',
    )!;
    expect(swift.name).toBe('照影游尘');
    expect(swift.description).toBe(
      '器随身走，身随势变；以迅疾身段连缀攻势，在交锋之间留下戏痕，最终将诸般戏影收束于《谢幕》。',
    );
  ...
  it('未激活流派使用基础戏念和基础灯律', () => {
    const projection = projectSectCombat({ sect: state(), realm: '守灯' })!;
    ...
    expect(projection.defaultAttack?.name).toBe('亮相');
  });
  ...
  it('照影游尘通过统一解析器生成变体与节点效果', () => {
    ...
    expect(detail.name).toBe('走场');
    expect(detail.summary).toBe('器锋纵横，数势相连；前器未尽，后器已越其锋。');
  ...
  it('守拙藏锋沿用宗门戏念并生成独立技能变体和策略', () => {
    ...
    expect(projection.selectionStrategy).toBeDefined();
    expect(
      resolveSectAbility({ sect, realm: '忘川', abilityId: 'linked-edge' })
        .name,
    ).toBe('走场');
    expect(
  ...
  it('流派基础变体不再随已解锁层数改变倍率', () => {
    for (const pathId of ['swift-sword', 'heavy-sword'] as const) {
      ...
      expect(
        productionSectRuntime.compiler.compile(BAIXIBAN_MODULE, {
          sect: firstLayer,
          realm: '忘川',
        }),
      ).toEqual(
  ...
  it.each(['swift-sword', 'heavy-sword'] as const)(
    ...
    (pathId) => {
      const path = BAIXIBAN_MODULE.definition.paths.find(
  ...
  it.each([
    ...
    (level, duration, value) => {
      ...
      const compiled = productionSectRuntime.compiler.compile(BAIXIBAN_MODULE, {
        sect,
        realm: '忘川',
      });
      ...
      const swordMark = findObjectById(probing, 'sect.baixiban.sword-mark');
  ...
  it('最终神通事实正确合并节点倍率、门槛与跨神通被动', () => {
    ...
    expect(shadow.detailRows).toContain('施放条件：至少6点戏念');
    ...
    expect(returningPeak.detailRows).toEqual(
      expect.arrayContaining([
        '伤害：基础相当于80.3%物攻，每点戏念增加26.46%物攻',
        '命中后：戏念：返还2点',
        '命中后：护盾：相当于38.65%物攻',
      ]),
    );
    ...
    expect(mountainBreaking.detailRows).toEqual(
      expect.arrayContaining([
        '命中后：状态：消耗全部戏痕',
        '命中后：每消耗1层戏痕，额外造成相当于12.88%物攻的伤害',
      ]),
    );
  ...
  it.each([undefined, 'swift-sword', 'heavy-sword'] as const)(
    ...
    (pathId) => {
      const sect = state(pathId);
      for (const definition of BAIXIBAN_MODULE.definition.abilities.filter(
  ...
  it.each([
    ...
    projectSectCombat({ sect: state(pathId, nodes), realm: '忘川' });
  ...
  it('动态详情保留条件、目标、触发次数与统一术语', () => {
    ...
    expect(
      ...
      '命中后：触发：持续期间首次闪避时，反击造成相当于43.44%物攻的伤害，并获得1点戏念',
    );
    expect(
      resolveSectAbility({
        sect: swift,
    ...
    ).toContain('施展后：触发：持续期间首次闪避时，获得1点戏念');
    expect(
      resolveSectAbility({
        sect: swift,
        realm: '忘川',
    ...
    ).toContain('命中后：消耗全部戏念');
  });

  it('动态详情聚合戏痕并完整描述节点触发效果', () => {
    const swift = state('swift-sword', [
    ...
    ).toContain('命中后：戏痕：向目标施加2层，持续目标未来4次行动');
    const swordMarkRows = resolveSectAbility({
      sect: swift,
      realm: '忘川',
      abilityId: 'linked-edge',
    }).detailRows;
    expect(swordMarkRows).toContain('命中后：戏痕：最多3层');
    ...
    expect(
      ...
      '命中后：触发：持续期间首次闪避时，反击造成相当于43.44%物攻的伤害、获得1点戏念、向目标施加1层戏痕，持续目标未来4次行动，并获得相当于43.44%物攻的护盾',
    );
