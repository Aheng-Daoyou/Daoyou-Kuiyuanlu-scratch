...
import {
  BAIXIBAN_HEAVY_PATH_MODULE,
  BAIXIBAN_SWIFT_PATH_MODULE,
  BAIXIBAN_STAGE_GRACE,
  BaixibanHeavySelectionStrategy,
  BaixibanSwiftSelectionStrategy,
} from '..';
...
function state(pathId: PathId): CultivatorSectState {
  return {
    membershipId: 'baixiban-strategy',
    sectId: 'baixiban',
    status: 'active',
    contribution: 0,
    configVersion: 4,
...
describe('流派策略插件', () => {
  it('按流派创建互相独立的策略', () => {
    expect(
      BAIXIBAN_SWIFT_PATH_MODULE.createSelectionStrategy('aggressive'),
    ).toBeInstanceOf(BaixibanSwiftSelectionStrategy);
    expect(
      BAIXIBAN_HEAVY_PATH_MODULE.createSelectionStrategy('heavy-break'),
    ).toBeInstanceOf(BaixibanHeavySelectionStrategy);
  });

  it('两个流派的六个战术都由各自模块接受并创建策略', () => {
    for (const tactic of BAIXIBAN_SWIFT_PATH_MODULE.definition.tactics) {
      expect(
        BAIXIBAN_SWIFT_PATH_MODULE.createSelectionStrategy(tactic.id),
      ).toBeInstanceOf(BaixibanSwiftSelectionStrategy);
    }
    for (const tactic of BAIXIBAN_HEAVY_PATH_MODULE.definition.tactics) {
      expect(
        BAIXIBAN_HEAVY_PATH_MODULE.createSelectionStrategy(tactic.id),
      ).toBeInstanceOf(BaixibanHeavySelectionStrategy);
    }
  });
  ...
  it.each([
    [
      '照影游尘',
      'swift-sword',
      new BaixibanSwiftSelectionStrategy('aggressive'),
    ],
    [
      '守拙藏锋',
      'heavy-sword',
      new BaixibanHeavySelectionStrategy('heavy-break'),
    ],
  ] as const)(
    '%s在目标存在可驱散增益时使用破台',
  ...
  it('快器普通回退使用通用评分，不因槽位顺序改变结果', () => {
    const strategy = new BaixibanSwiftSelectionStrategy('aggressive');
    ...
    expect(first?.ability.id).toBe('sect.baixiban.breaking-edge');
